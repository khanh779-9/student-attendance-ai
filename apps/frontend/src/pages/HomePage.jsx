  ...
  return (
    <section className="rounded-panel border border-slate-200 bg-white p-5 shadow-panel sm:p-6">
      ...
      <p className="mt-4 text-sm text-slate-600">
        {classMessage || "Các lớp học của bạn sẽ hiển thị ở đây"}
      </p>

      <div className="mt-5 grid grid-cols-1 gap-4 xl:grid-cols-2">
        {classRows.map((course) => (
          <article
            className="rounded-xl border border-slate-200 bg-slate-50 p-4 shadow-md hover:shadow-lg transition-shadow"
            key={course.id}
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="text-sm text-slate-500">{course.id}</p>
              </div>
